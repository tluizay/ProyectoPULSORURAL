...
function inicializarMapaSigpac(config) {
    // Si el DOM ya está listo (por ejemplo si el script se carga con defer o de forma
    // diferida), DOMContentLoaded ya habría pasado y el mapa no se montaría nunca.
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", function () {
            montarMapaSigpac(config);
        });
    } else {
        montarMapaSigpac(config);
    }
...
function montarMapaSigpac(config) {
    // Comprobaciones previas: si falta Leaflet o el contenedor, avisamos en consola
    // en vez de lanzar una excepción silenciosa que deje la página a medias.
    if (typeof L === "undefined") {
        console.error("Leaflet (L) no está cargado. Revisa el <script> de leaflet.js en base.html.");
        return;
    }

    const contenedor = document.getElementById("contenedor-mapa");
    if (!contenedor) {
        console.error("No existe el div #contenedor-mapa en la plantilla.");
        return;
    }
    ...
    try {
        if (L.Control && typeof L.Control.geocoder === "function") {
            ...
            console.warn("L.Control.geocoder no está disponible; el mapa funciona pero sin buscador.");
        }
    } catch (errorGeocoder) {
    ...
    // Recalcular el tamaño: al arrancar, al redimensionar la ventana y al cerrar
    // el modal de bienvenida (si el mapa se monta oculto, Leaflet calcula mal las losetas).
    setTimeout(function () {
        mapa.invalidateSize();
    }, 200);
    ...
    const modalBienvenida = document.getElementById("welcomeModal");
    if (modalBienvenida) {
        modalBienvenida.addEventListener("hidden.bs.modal", function () {
            mapa.invalidateSize();
        });
    }
    ...
    // String() protege por si Django inyecta un número en lugar de una cadena.
    const lat = String(config.lat).replace(',', '.');
    const lon = String(config.lon).replace(',', '.');

    // A. AEMET
    const urlClima = config.urlClima || '/aemet/consulta_clima/';
    fetch(`${urlClima}?lat=${lat}&lon=${lon}`)
        .then(response => {
